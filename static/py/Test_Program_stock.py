...
import inspect
#-----------------------------------------------------
import realTimeDataIB, trailValueIb, lastFromRealTimeIb, stopRecommendIb # my program that does the computation
import math
#--------------------------------------------------------
import logging
import time
...
class TestApp(TestWrapper, TestClient):
    def __init__(self):
        ...
        self.tickr = ContractSamples.USStock().symbol # my add,
...
#
    ...
    def realTimeBars_req(self):
        ...
        print (self.reqRealTimeBars(3001, ContractSamples.USStock(), 5, "MIDPOINT", True, []))
        # ! [reqrealtimebars]
    ...
    def bracketSample(self, parentOrderId:int, action:str, quantity:float, takeProfitLimitPrice:float,
                     stopLossPrice:float):
        # BRACKET ORDER
        # ! [bracketsubmit]
        bracket = OrderSamples.BracketOrder(parentOrderId, action, quantity, takeProfitLimitPrice,
                         stopLossPrice)
        for o in bracket:
            self.placeOrder(o.orderId, ContractSamples.EurGbpFx(), o)
    ...
    def orderOperations_req(self):
        ...
        if self.positionExist == 0.0:
            ...
            buyOrSell = realTimeDataIB.buySellRecommendation(self.tickr)
            print (buyOrSell)
            if  buyOrSell[0] == "Buy":
                print (round(buyOrSell[2], 2))
                print (round(buyOrSell[1], 2))
                self.placeOrder(self.nextOrderId(), ContractSamples.USStock(),
                                OrderSamples.MarketOrder("BUY", 50))
                trailStopPrice = round(buyOrSell[1], 2)
                trailPercent = round(buyOrSell[2], 2)
                if trailPercent == 0.00:
                    trailPercent = 0.01
                limitPrice = round(trailStopPrice+trailPercent*3, 4)
                #-----------One cancels all-------------------------
                ocaOrders = []
                ocaOrders.append(OrderSamples.TrailingStop("SELL", 50, trailPercent, trailStopPrice))
                ocaOrders.append(OrderSamples.LimitOrder("SELL", 50, limitPrice))
                OrderSamples.OneCancelsAll("TestOCA_" + str(self.nextValidOrderId), ocaOrders, 2)
                for o in ocaOrders:
                    self.placeOrder(self.nextOrderId(), ContractSamples.USStock(), o)
                #-------------------OCA end-------------------
                # self.placeOrder(self.nextOrderId(), ContractSamples.USStock(),
                #                 OrderSamples.TrailingStop("SELL", 50, trailPercent, trailStopPrice)) #(action:str, quantity:float, lmtPriceOffset:float, trailingAmount:float, trailStopPrice:float):
            elif buyOrSell[0] == "Sell":
                print (round(buyOrSell[2], 2))
                print (round(buyOrSell[1], 2))
                self.placeOrder(self.nextOrderId(), ContractSamples.USStock(),
                                OrderSamples.MarketOrder("SELL", 50))
                trailStopPrice = round(buyOrSell[1], 2)
                trailPercent = round(buyOrSell[2], 2)
                if trailPercent == 0.00:
                    trailPercent = 0.01
                ...
                #-----------One cancels all-------------------------
                ocaOrders = []
                ocaOrders.append(OrderSamples.TrailingStop("BUY", 50, trailPercent, trailStopPrice))
                ocaOrders.append(OrderSamples.LimitOrder("BUY", 50, limitPrice))
                OrderSamples.OneCancelsAll("TestOCA_" + str(self.nextValidOrderId), ocaOrders, 2)
                for o in ocaOrders:
                    self.placeOrder(self.nextOrderId(),ContractSamples.USStock(), o)
                #-------------------OCA end-------------------
                # self.placeOrder(self.nextOrderId(), ContractSamples.USStock(),
                #                 OrderSamples.TrailingStop("BUY", 50, trailPercent, trailStopPrice))
        # ! [order_submission]
