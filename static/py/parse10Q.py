...
def parse_form_13F():
    ...
    instituionsFname = "csv\\StockOwnership\\stocks.csv"
    ...
    try:
        for filename in myFiles:
            df13Fdata = pd.read_csv(filename, encoding = "ISO-8859-1")
            list13FCiks.extend(df13Fdata['cik'].tolist())
    except:
        df13Fdata = pd.DataFrame(columns=columns)

    cikList = tuple(institutionsDf['cik'].tolist()) # sets and dicts have no index
    instNameList = tuple(institutionsDf['name'].tolist())
    set13FCiks = set(list13FCiks)
                        ...
                        if value[0].startswith("0"):
                            ...
                            if value[0].startswith("0"):
                                ...
                                if value[0].startswith("0"):
                                    value[0] = value[0][1:]
                        try:
                            targetList = [i for i in listOfdataList if cik in i and value[0].upper() in i]
                            indexToInsert = listOfdataList.index(targetList[0])
                            indexInTargetList = columns.index(filingDate)
                            sharesAlreadyOwn = targetList[0][indexInTargetList]
                            listOfdataList[indexToInsert][indexInTargetList] = sharesAlreadyOwn + int(value[1])
                        except:
                            dataList = [value[0].upper(), secName, int(cik), instNameList[ind]]+[0]*len(monthlist)
